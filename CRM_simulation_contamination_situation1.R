...
# Here, only the amount of casewise contamination is increased. This corresponds to situation 1.
rm(list = ls())
...
for (j_sim in 1:n_sims) {
  ...
  results_MAE_j_sim       <- data.frame(matrix(nrow = n_pct_case_out, ncol = 6))
  results_MSEP_j_sim      <- data.frame(matrix(nrow = n_pct_case_out, ncol = 6))
  results_precision_j_sim <- data.frame(matrix(nrow = n_pct_case_out, ncol = 6))
  results_recall_j_sim    <- data.frame(matrix(nrow = n_pct_case_out, ncol = 6))
  results_time_j_sim      <- data.frame(matrix(nrow = n_pct_case_out, ncol = 6))
  ...
  for (case_pct_index in 1:n_pct_case_out) {
    ...
    # For each new added casewise outlier contamination is added to pct_cell_out of the cells -----
    cell_outliers <- NULL
    ...
    # Mean Absolute Error - - - - - - - - - - - -
    results_MAE_j_sim$'CRM-Hampel-norm'[case_pct_index]     <- mean(abs(crm_hampel_norm$coefficients - betas_list[[j_sim]]))
    results_MAE_j_sim$'CRM-Hampel'[case_pct_index]          <- mean(abs(crm_hampel$coefficients - betas_list[[j_sim]]))
    results_MAE_j_sim$'CRM-Tukey'[case_pct_index]           <- mean(abs(crm_tukey$coefficients - betas_list[[j_sim]]))
    results_MAE_j_sim$'CRM-Huber'[case_pct_index]           <- mean(abs(crm_huber$coefficients - betas_list[[j_sim]]))
    results_MAE_j_sim$'CRM-Gauss'[case_pct_index]           <- mean(abs(crm_gauss$coefficients - betas_list[[j_sim]]))
    results_MAE_j_sim$'CRM-Quadratic'[case_pct_index]       <- mean(abs(crm_quadratic$coefficients - betas_list[[j_sim]]))
    
    
    # Mean Squared Error of Prediction - - - - - -
    if (length(case_outliers) == 0) {
      results_MSEP_j_sim$'CRM-Hampel-norm'[case_pct_index]      <- mean((crm_hampel_norm$fitted.values - data_clean$y)^2)
      results_MSEP_j_sim$'CRM-Hampel'[case_pct_index]           <- mean((crm_hampel$fitted.values - data_clean$y)^2)
      results_MSEP_j_sim$'CRM-Tukey'[case_pct_index]            <- mean((crm_tukey$fitted.values - data_clean$y)^2)
      results_MSEP_j_sim$'CRM-Huber'[case_pct_index]            <- mean((crm_huber$fitted.values - data_clean$y)^2)
      results_MSEP_j_sim$'CRM-Gauss'[case_pct_index]            <- mean((crm_gauss$fitted.values - data_clean$y)^2)
      results_MSEP_j_sim$'CRM-Quadratic'[case_pct_index]        <- mean((crm_quadratic$fitted.values - data_clean$y)^2)
    } else {
      results_MSEP_j_sim$'CRM-Hampel-norm'[case_pct_index]      <- mean((crm_hampel_norm$fitted.values - data_clean$y)[-case_outliers]^2)
      results_MSEP_j_sim$'CRM-Hampel'[case_pct_index]           <- mean((crm_hampel$fitted.values - data_clean$y)[-case_outliers]^2)
      results_MSEP_j_sim$'CRM-Tukey'[case_pct_index]            <- mean((crm_tukey$fitted.values - data_clean$y)[-case_outliers]^2)
      results_MSEP_j_sim$'CRM-Huber'[case_pct_index]            <- mean((crm_huber$fitted.values - data_clean$y)[-case_outliers]^2)
      results_MSEP_j_sim$'CRM-Gauss'[case_pct_index]            <- mean((crm_gauss$fitted.values - data_clean$y)[-case_outliers]^2)
      results_MSEP_j_sim$'CRM-Quadratic'[case_pct_index]        <- mean((crm_quadratic$fitted.values - data_clean$y)[-case_outliers]^2)
    }
    
    
    # Precision & recall of detected cells - - - -
    if (length(case_outliers) == 0) {
      results_precision_j_sim$'CRM-Hampel-norm'[case_pct_index]   <- NA
      results_precision_j_sim$'CRM-Hampel'[case_pct_index]        <- NA
      results_precision_j_sim$'CRM-Tukey'[case_pct_index]         <- NA
      results_precision_j_sim$'CRM-Huber'[case_pct_index]         <- NA
      results_precision_j_sim$'CRM-Gauss'[case_pct_index]         <- NA
      results_precision_j_sim$'CRM-Quadratic'[case_pct_index]     <- NA
      
      results_recall_j_sim$'CRM-Hampel-norm'[case_pct_index]      <- NA
      results_recall_j_sim$'CRM-Hampel'[case_pct_index]           <- NA
      results_recall_j_sim$'CRM-Tukey'[case_pct_index]            <- NA
      results_recall_j_sim$'CRM-Huber'[case_pct_index]            <- NA
      results_recall_j_sim$'CRM-Gauss'[case_pct_index]            <- NA
      results_recall_j_sim$'CRM-Quadratic'[case_pct_index]        <- NA
    } else {
      ...
      results_precision_j_sim$'CRM-Hampel-norm'[case_pct_index]   <- CM_hampel_norm[2, 2] / (CM_hampel_norm[2, 1] + CM_hampel_norm[2, 2])
      results_precision_j_sim$'CRM-Hampel'[case_pct_index]        <- CM_hampel[2, 2] / (CM_hampel[2, 1] + CM_hampel[2, 2])
      results_precision_j_sim$'CRM-Tukey'[case_pct_index]         <- CM_tukey[2, 2] / (CM_tukey[2, 1] + CM_tukey[2, 2])
      results_precision_j_sim$'CRM-Huber'[case_pct_index]         <- CM_huber[2, 2] / (CM_huber[2, 1] + CM_huber[2, 2])
      results_precision_j_sim$'CRM-Gauss'[case_pct_index]         <- CM_gauss[2, 2] / (CM_gauss[2, 1] + CM_gauss[2, 2])
      results_precision_j_sim$'CRM-Quadratic'[case_pct_index]     <- CM_quadratic[2, 2] / (CM_quadratic[2, 1] + CM_quadratic[2, 2])
      
      results_recall_j_sim$'CRM-Hampel-norm'[case_pct_index]      <- CM_hampel_norm[2, 2] / (CM_hampel_norm[1, 2] + CM_hampel_norm[2, 2])
      results_recall_j_sim$'CRM-Hampel'[case_pct_index]           <- CM_hampel[2, 2] / (CM_hampel[1, 2] + CM_hampel[2, 2])
      results_recall_j_sim$'CRM-Tukey'[case_pct_index]            <- CM_tukey[2, 2] / (CM_tukey[1, 2] + CM_tukey[2, 2])
      results_recall_j_sim$'CRM-Huber'[case_pct_index]            <- CM_huber[2, 2] / (CM_huber[1, 2] + CM_huber[2, 2])
      results_recall_j_sim$'CRM-Gauss'[case_pct_index]            <- CM_gauss[2, 2] / (CM_gauss[1, 2] + CM_gauss[2, 2])
      results_recall_j_sim$'CRM-Quadratic'[case_pct_index]        <- CM_quadratic[2, 2] / (CM_quadratic[1, 2] + CM_quadratic[2, 2])
    }
    
    
    # Execution time - - - - - - - - - - - - - - -
    results_time_j_sim$'CRM-Hampel-norm'[case_pct_index]    <- crm_hampel_norm$time
    results_time_j_sim$'CRM-Hampel'[case_pct_index]         <- crm_hampel$time
    results_time_j_sim$'CRM-Tukey'[case_pct_index]          <- crm_tukey$time
    results_time_j_sim$'CRM-Huber'[case_pct_index]          <- crm_huber$time
    results_time_j_sim$'CRM-Gauss'[case_pct_index]          <- crm_gauss$time
    results_time_j_sim$'CRM-Quadratic'[case_pct_index]      <- crm_quadratic$time
...
# Mean Absolute Error - - - - - - - - - - - - - -
plot(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise outliers",
     ylab = "Average MAE", ylim = range(results_MAE_mean[, -1]), las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2)
lines(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Hampel', lwd = 2, lty = 2)
lines(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Tukey',      lwd = 2, lty = 3)
lines(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Huber',  lwd = 2, lty = 4)
lines(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Gauss',     lwd = 2, lty = 5)
lines(100 * results_MAE_mean$pct_case_out, results_MAE_mean$'CRM-Quadratic', lwd = 2, lty = 6)
legend("bottomright", legend = names(results_MAE_mean)[-1], lty = 1:6, lwd = 2, cex = 1, y.intersp = 0.5)


# Mean Squared Error of Prediction - - - - - - - -
plot(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise outliers",
     ylab = "Average MSEP", ylim = range(results_MSEP_mean[, -1]), las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2)
lines(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Hampel', lwd = 2, lty = 2)
lines(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Tukey',      lwd = 2, lty = 3)
lines(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Huber',  lwd = 2, lty = 4)
lines(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Gauss',     lwd = 2, lty = 5)
lines(100 * results_MSEP_mean$pct_case_out, results_MSEP_mean$'CRM-Quadratic', lwd = 2, lty = 6)
legend("topleft", legend = names(results_MSEP_mean)[-1], lty = 1:6, lwd = 2)


# Precision detected cells - - - - - - - - - - - -
plot(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise outliers",
     ylab = "Average precision", las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2, ylim = c(0, 1))
lines(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Hampel', lwd = 2, lty = 2)
lines(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Tukey',      lwd = 2, lty = 3)
lines(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Huber',  lwd = 2, lty = 4)
lines(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Gauss',     lwd = 2, lty = 5)
lines(100 * results_precision_mean$pct_case_out, results_precision_mean$'CRM-Quadratic', lwd = 2, lty = 6)
legend("topleft", legend = names(results_precision_mean)[-1], lty = 1:6, lwd = 2)


# Recall of detected cells - - - - - - - - - - - -
plot(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise outliers",
     ylab = "Average recall", las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2, ylim = c(0, 1))
lines(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Hampel', lwd = 2, lty = 2)
lines(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Tukey',      lwd = 2, lty = 3)
lines(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Huber',  lwd = 2, lty = 4)
lines(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Gauss',     lwd = 2, lty = 5)
lines(100 * results_recall_mean$pct_case_out, results_recall_mean$'CRM-Quadratic', lwd = 2, lty = 6)
...
MAE_matrix <- data.frame(pct_case_out = rep(100 * results_MAE_mean$pct_case_out, times = 6),
...
tmp %>%
  ggplot(aes(x = pct_case_out, y = average_MAE)) +
  ...
  ylab("Average MAE") + xlab("% casewise contamination") +
