...
# Here, both the amount of casewise and cellwise contamination is increased. This corresponds to situation 3.
rm(list = ls())
...
for (j_sim in 1:n_sims) {
  ...
  for (case_pct_index in 1:n_pct_case_out) {
    ...
    # For the new casewise outliers, cellwise contamination is added according to the current percentage of cellwise outliers
    for (j in new_case_outliers) {
      pct_cell_out <- pct_cell_out_seq[cell_pct_index]
      
      new_cell_outliers[[j]] <- sample(uncontaminated_cells_list[[j]], size = round(p * pct_cell_out))
      
      cell_outliers       <- c(cell_outliers, new_cell_outliers[[j]])
      uncontaminated_cells_list[[j]] <- setdiff(uncontaminated_cells_list[[j]], new_cell_outliers[[j]])
      
      Xc_list[[j_sim]][j, new_cell_outliers[[j]]] <- contamination[new_cell_outliers[[j]]] + rnorm(length(new_cell_outliers[[j]]))
      outliers_mat_flag[j, new_cell_outliers[[j]]] <- TRUE
    }
    
    # For the already existing casewise outliers, cellwise contamination is increased in a step of 5% additional cellwise contamination
    for (k in case_outliers) {
      extra_pct_cell_out <- ifelse(cell_pct_index == 1,
                                   pct_cell_out_seq[1],
                                   pct_cell_out_seq[cell_pct_index] - pct_cell_out_seq[cell_pct_index - 1])
      
      new_cell_outliers[[k]] <- sample(uncontaminated_cells_list[[k]], size = round(p * extra_pct_cell_out))
      
      cell_outliers       <- c(cell_outliers, new_cell_outliers[[k]])
      uncontaminated_cells_list[[k]] <- setdiff(uncontaminated_cells_list[[k]], new_cell_outliers[[k]])
      
      Xc_list[[j_sim]][k, new_cell_outliers[[k]]] <- contamination[new_cell_outliers[[k]]] + rnorm(length(new_cell_outliers[[k]]))
      outliers_mat_flag[k, new_cell_outliers[[k]]] <- TRUE
    }
...
# Mean Absolute Error - - - - - - - - - - - - - -
plot(100 * results_MAE_mean$pct_cell_out, results_MAE_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise and cellwise outliers",
...
# Mean Squared Error of Prediction - - - - - - - -
plot(100 * results_MSEP_mean$pct_cell_out, results_MSEP_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise and cellwise outliers",
...
plot(100 * results_precision_mean$pct_cell_out, results_precision_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise and cellwise outliers",
...
plot(100 * results_recall_mean$pct_cell_out, results_recall_mean$'CRM-Hampel-norm', type = "l", xlab = "% casewise and cellwise outliers",
...
tmp %>%
  ...
  ylab("Average MAE") + xlab("% casewise and cellwise contamination") +
